...
const About = ({inverted, subheading, heading, text, buttonText, buttonLink, image} : {inverted?: boolean, image: StaticImageData, subheading: string, heading: string, text: string, buttonText: string, buttonLink: string}) => {
  return (
    <div className={`relative max-w-[1260px] flex flex-col lg:flex-row items-center justify-center mx-auto p-3 sm:p-6 md:p-12`}>
      {/* Text Box */}
      <div className={`relative  mt-4 xs:mt-0 p-[5%] xs:p-[10%] sm:p-[15%] lg:p-[5rem] bg-white shadow-xl flex-1  flex items-center min-h-[calc(100%-20px)] order-2 ${inverted ? " lg:ml-[-1.5rem] lg:mr-6 z-[-1]" : "lg:order-1  lg:mr-[-1.5rem] z-10"}`}>
      ...
      {/* Image */}
      <div className={`relative overflow-hidden lg:overflow-visible mt-6 md:mt-0 w-full lg:w-1/2 order-1 ${inverted ? " z-[-1]" : "lg:order-2"}`}>
